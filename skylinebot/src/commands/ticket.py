...
class Ticket(commands.Cog):
    ...
    @commands.hybrid_group(
        name="ticket",
        help="ชุดคำสั่งสำหรับจัดการทิกเก็ต",
        aliases=["tickets"],
    ...
    async def ticket(self, ctx):
        try:
            embed = discord.Embed(
                title="คำสั่งทิกเก็ต",
                description="ใช้คำสั่งเหล่านี้เพื่อจัดการทิกเก็ต",
    ...
    @ticket.command(
        name='setup',
        help="ตั้งค่าระบบทิกเก็ตในเซิร์ฟเวอร์นี้",
        with_app_command=True
    )
    @checks.ignore_check()
    @checks.blacklist_check()
        ...
        try:
            ...
            async def get_embed():
                try:
                    ...
                    embed = discord.Embed(
                        title="ตั้งค่าทิกเก็ต",
                        description="คุณสามารถตั้งค่าระบบทิกเก็ตได้ที่นี่\n\n",
                        color=color.green
                    )
...
> {self.bot.emoji.ROLE} **ช่วยเหลือ Roles:** {support_roles_text}
...
"""
            ...
            async def create_button_callback(interaction:discord.Interaction):
                try:
                    if interaction.user.id != ctx.author.id:
                        return await interaction.response.send_message(embed=discord.Embed(description="คุณไม่มีสิทธิ์ใช้ปุ่มนี้",color=color.red),ephemeral=True,delete_after=10)
            ...
            async def edit_select_callback(interaction:discord.Interaction):
                try:
                    ...
                    if not selected_values:
                        return await interaction.response.send_message(embed=discord.Embed(description="ยังไม่ได้เลือกโมดูลทิกเก็ต",color=color.red),ephemeral=True,delete_after=10)
                    ...
                    if not ticket_settings_data:
                        return await interaction.response.send_message(embed=discord.Embed(description="ไม่พบโมดูลทิกเก็ต",color=color.red),ephemeral=True,delete_after=10)
                    ...
                    async def get_selected_edit_embed():
                        ...
                        if not ticket_settings_data:
                            return discord.Embed(
                                ...
                                description="ไม่พบโมดูลทิกเก็ต",
                                color=color.red
                            )
                        embed = discord.Embed(
                            title=F"Edit Ticket Module {MODULE_ID}",
                            description="คุณสามารถแก้ไขโมดูลทิกเก็ตได้ที่นี่",
                            color=color.green
                        )
                        embed.add_field(
                            name="Enabled",
                    ...
                    async def get_selected_view(disabled:bool=False, summary_text:str=""):
                        ...
                        enable_disable_button = discord.ui.Button(
                            label="Enable Module" if not ticket_settings_data.get('enabled',False) else "Disable Module",
                        ...
                        support_roles = discord.ui.RoleSelect(
                            placeholder="Select ช่วยเหลือ Roles",
                            max_values=10,
                            min_values=0,
                    ...
                    async def edit_ticket_limit_button_callback(interaction:discord.Interaction):
                        try:
                            ...
                            class set_ticket_limit(discord.ui.Modal,title="กำหนดวงเงินตั๋ว"):
                                ...
                                async def on_submit(self, interaction: discord.Interaction):
                                    try:
                                        ...
                                        if new_ticket_limit < 1:
                                            return await interaction.response.send_message(embed=discord.Embed(description="จำนวนทิกเก็ตสูงสุดต้องมากกว่า 0",color=color.red),ephemeral=True,delete_after=10)
                                        ticket_settings_data = cache.ticket_settings.get(str(ctx.guild.id),{}).get(MODULE_ID,{})
                                        if not ticket_settings_data:
                                            return await interaction.response.send_message(embed=discord.Embed(description="ไม่พบโมดูลทิกเก็ต",color=color.red),ephemeral=True,delete_after=10)
                                        await interaction.response.defer()
    ...
    async def ticket_close(self,ctx: commands.Context,module_id:int=None,ticket_id:int=None):
        try:
            ...
            if not module_id and not ticket_id:
                ...
                if not ticket_data:
                    return await self._safe_ctx_send(ctx, embed=discord.Embed(description="ช่องนี้ไม่ใช่ช่องทิกเก็ต",color=color.red))
            else:
                ticket_data = await storage.tickets.get(guild_id=ctx.guild.id,ticket_module_id=module_id,ticket_id=ticket_id)
                if not ticket_data:
                    return await self._safe_ctx_send(ctx, embed=discord.Embed(description="ไม่พบทิกเก็ต",color=color.red))
            ...
            if await ticket_panel.ticket_close_action(guild=ctx.guild,ticket_data=ticket_data,bot=self.bot,closed_by=ctx.author):
                await self._safe_ctx_send(ctx, embed=discord.Embed(description="ปิดทิกเก็ตเรียบร้อยแล้ว",color=color.green))
            else:
                await self._safe_ctx_send(ctx, embed=discord.Embed(description="ปิดทิกเก็ตไม่สำเร็จ",color=color.red))
        except Exception as e:
            logger.error(f"ข้อผิดพลาด in file {__file__} at line {traceback.extract_tb(sys.exc_info()[2])[0][1]}: {e}")
    ...
    async def ticket_delete(self,ctx: commands.Context,module_id:int=None,ticket_id:int=None):
        try:
            ...
            embed = discord.Embed(
                title="ยืนยันการลบ",
                description="คุณต้องการลบช่องทิกเก็ตหรือไม่?",
                color=color.red
            )
