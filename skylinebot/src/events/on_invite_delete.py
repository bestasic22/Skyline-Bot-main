    ...
    async def invite_delete_log(self,invite:discord.Invite):
        try:
            ...
            if not guilds_log_cache.get('enabled'):
                return logger.debug(f"Guild {invite.guild.name} has logging disabled")
            channel_id = guilds_log_cache.get('invite_delete_channel_id')
            if not channel_id:
                return logger.debug(f"Channel ID not found for invite delete log in {invite.guild.name}")
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{invite.guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Invite Delete\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=invite.guild.icon.url if invite.guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{invite.guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Invite Delete\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=invite.guild.icon.url if invite.guild.icon else None)
                                ...
                                return logger.warning(f"การดำเนินการไม่ถูกต้อง {action} in {invite.guild.name}")
