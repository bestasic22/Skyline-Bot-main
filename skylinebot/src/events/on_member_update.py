    ...
    async def member_update_log(self, member_before: discord.Member, member_after: discord.Member):
        try:
            ...
            if not guilds_log_cache.get('enabled'):
                return logger.debug(f"Guild {member_after.guild.name} has logging disabled")
            channel_id = guilds_log_cache.get('member_update_channel_id')
            if not channel_id:
                return logger.debug(f"Channel ID not found for member update log in {member_after.guild.name}")
    ...
    async def anti_update_module(self,member:discord.Member,member_after:discord.Member):
        try:
            ...
            if member.roles == member_after.roles:
                return logger.info(f"Skip anti member update for {member}: no role delta")
            ...
            if entry:
                ...
                if bool(getattr(updater, "bot", False)) or bool(getattr(updater, "system", False)):
                    return logger.info(
                        f"Skip anti member update for bot/system actor {updater} in {member.guild.name}"
                    )
            else:
                return logger.info(f"User {member} update audit entry not found (likely system/integration)")

            if not isinstance(updater, discord.Member):
                resolved_member = member.guild.get_member(getattr(updater, "id", 0))
                if resolved_member is None:
                    try:
                        resolved_member = await member.guild.fetch_member(getattr(updater, "id", 0))
                    except Exception:
                        resolved_member = None
                if resolved_member is None:
                    return logger.info(
                        f"Skip anti member update: actor is not a guild member (likely integration/system) in {member.guild.name}"
                    )
                updater = resolved_member
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{member.guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Member Update\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=member.guild.icon.url if member.guild.icon else None)
                                ...
                                return logger.warning(f"การดำเนินการไม่ถูกต้อง {action} in {member.guild.name}")
