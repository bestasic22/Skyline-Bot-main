    ...
    async def webhooks_update_log(self,channel:discord.TextChannel):
        try:
            ...
            if not guilds_log_cache.get('enabled'):
                return logger.debug(f"Guild {channel.guild.name} has logging disabled")
            ...
            if created:
                ...
                if not channel_id:
                    return logger.debug(f"Channel ID not found for webhook create log in {channel.guild.name}")
            ...
            if removed:
                ...
                if not channel_id:
                    return logger.debug(f"Channel ID not found for webhook delete log in {channel.guild.name}")
            ...
            if updated:
                ...
                if not channel_id:
                    return logger.debug(f"Channel ID not found for webhook update log in {channel.guild.name}")
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{webhook.guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Webhook Create\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=webhook.guild.icon.url if webhook.guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{webhook.guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Webhook Create\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=webhook.guild.icon.url if webhook.guild.icon else None)
                                ...
                                return logger.warning(f"การดำเนินการไม่ถูกต้อง {action} in {webhook.guild.name}")
                        ...
                        if (self.delete_webhook_timeouts.get(str(webhook.guild.id),{}).get(str(deleter.id),{}).get('count') >= anti_nuke_cache.get('anti_webhook_delete_limit',1)
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Warning",
                                        description=f"**__Guild:__ `{webhook.guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Webhook Delete\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=webhook.guild.icon.url if webhook.guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Restriction",
                                        description=f"**__Guild:__ `{webhook.guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Webhook Delete\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=webhook.guild.icon.url if webhook.guild.icon else None)
                        ...
                        if (self.update_webhook_timeouts.get(str(webhook.guild.id),{}).get(str(updater.id),{}).get('count') >= anti_nuke_cache.get('anti_webhook_update_limit',1)
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Warning",
                                        description=f"**__Guild:__ `{webhook.guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Webhook Update\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=webhook.guild.icon.url if webhook.guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Restriction",
                                        description=f"**__Guild:__ `{webhook.guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Webhook Update\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=webhook.guild.icon.url if webhook.guild.icon else None)
