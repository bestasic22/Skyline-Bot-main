                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Emoji Create\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        ...
                                        description=f"**__Guild:__ `{guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Emoji Create\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
                                ...
                                return logger.warning(f"การดำเนินการไม่ถูกต้อง {action} in {guild.name}")
                        ...
                        if (self.delete_emoji_timeouts.get(str(guild.id),{}).get(str(deleter.id),{}).get('count') >= anti_nuke_cache.get('anti_emote_delete_limit',1)
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Warning",
                                        description=f"**__Guild:__ `{guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Emoji Delete\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Restriction",
                                        description=f"**__Guild:__ `{guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Emoji Delete\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
                        ...
                        if (self.update_emoji_timeouts.get(str(guild.id),{}).get(str(updater.id),{}).get('count') >= anti_nuke_cache.get('anti_emote_update_limit',1)
                            ...
                            if action == 'ban':
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Warning",
                                        description=f"**__Guild:__ `{guild.name}`**\n**Details:** ```\nคุณได้รับคำเตือนจากระบบ: Anti Emoji Update\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
                                ...
                                try:
                                    embed = discord.Embed(
                                        title="Antinuke Restriction",
                                        description=f"**__Guild:__ `{guild.name}`**\n**Details:** ```\nคุณถูกปิดเสียงจากระบบ: Anti Emoji Update\nกรุณาอย่าทำซ้ำอีก\n```\n**__Time:__** <t:{int(datetime.datetime.now().timestamp())}:R>",
                                        color=color.red
                                    )
                                    embed.set_footer(text=f"Antinuke System",icon_url=self.bot.user.display_avatar.url)
                                    embed.set_thumbnail(url=guild.icon.url if guild.icon else None)
    ...
    async def emojis_add_or_remove_or_update_log(self, guild: discord.Guild, before: list[discord.Emoji], after: list[discord.Emoji]):
        try:
            ...
            if not guilds_log_cache.get('enabled'):
                return logger.debug(f"Guild {guild.name} has logging disabled")
            ...
            if added:
                ...
                if not channel_id:
                    return logger.debug(f"Channel ID not found for emojis add log in {guild.name}")
            ...
            if removed:
                ...
                if not channel_id:
                    return logger.debug(f"Channel ID not found for emojis remove log in {guild.name}")
            ...
            if updated:
                ...
                if not channel_id:
                    return logger.debug(f"Channel ID not found for emojis update log in {guild.name}")
