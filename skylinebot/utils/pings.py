...
from skylinebot.memory.cache import cache as cache_module
from skylinebot.bridge.storage import ping as ping_storage
...
def surface() -> float:
    start_time = time()
    try:
        requests.get(discord.http.Route.BASE, timeout=4)
        return round(((time() - start_time) * 1000), 2)
    except Exception:
        return -1
...
def youtube() -> float:
    start_time = time()
    try:
        requests.get("https://www.youtube.com", timeout=4)
        return round(((time() - start_time) * 1000), 2)
    except Exception:
        return -1
...
def github() -> float:
    start_time = time()
    try:
        requests.get("https://www.github.com", timeout=4)
        return round(((time() - start_time) * 1000), 2)
    except Exception:
        return -1
