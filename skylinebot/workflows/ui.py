...
SkylineBOT_DIR = Path(__file__).resolve().parents[1]
STYLE_DIR = SkylineBOT_DIR / "style"
FONTS_DIR = SkylineBOT_DIR / "fonts"
WINDOWS_FONTS_DIR = Path("C:/Windows/Fonts")
_IMAGE_FALLBACK_PATH = STYLE_DIR / "templates" / "music_background.png"
_WARNED_IMAGE_LOAD_URLS: set[str] = set()
...
def _font_candidates(text: str = "", bold: bool = False, font_style: str = "classic"):
    style_key = str(font_style or "classic").strip().lower()
    style_map = {
        "classic": ["arialbd.ttf", "arial.ttf"] if bold else ["arial.ttf", "arialbd.ttf"],
        "clean": ["DejaVuSans-Bold.ttf", "arialbd.ttf", "arial.ttf"] if bold else ["DejaVuSans-Bold.ttf", "arial.ttf", "arialbd.ttf"],
        "impact": ["ProtestGuerrilla-Regular.ttf", "arialbd.ttf", "arial.ttf"],
        "soft": ["SofadiOne-Regular.ttf", "arial.ttf", "arialbd.ttf"],
    }
    local_candidates = style_map.get(style_key, style_map["classic"])
    local_candidates = ["arialbd.ttf", "arial.ttf"] if bold else ["arial.ttf", "arialbd.ttf"]
    emoji_windows_candidates = ["seguiemj.ttf", "SegoeUIEmoji.ttf"]
    thai_windows_candidates = (
        ["LeelaUIb.ttf", "tahomabd.ttf", "LeelawUIb.ttf", "THSarabunNew Bold.ttf"]
        if bold
        else ["LeelawUI.ttf", "tahoma.ttf", "THSarabunNew.ttf", "NotoSansThai-Regular.ttf"]
    )
    fallback_candidates = ["DejaVuSans-Bold.ttf", "arial.ttf", "arialbd.ttf"]

    if _contains_thai(text):
        for name in thai_windows_candidates:
            yield WINDOWS_FONTS_DIR / name
    for name in emoji_windows_candidates:
        yield WINDOWS_FONTS_DIR / name
...
def load_ui_font(size: int, text: str = "", bold: bool = False, font_style: str = "classic"):
    for font_path in _font_candidates(text=text, bold=bold, font_style=font_style):
        try:
            if font_path.exists():
...
def get_ui_profile(
    ...
    display_name_x = 34
    display_name_y = 365
    display_name_font = load_ui_font(35, text=display_name, bold=True)
    ...
    username_x = 34
    username_y = 410
    username_font = load_ui_font(25, text=username, bold=False)
    ...
    userid_x = 34
    userid_y = 445
    userid_font = load_ui_font(20, text=userid, bold=False)
    ...
    coin_font = load_ui_font(20, text=coin_text, bold=True)
    ...
    created_at_x = 60
    created_at_y = 790
    created_at_font = load_ui_font(30, text=created_at_text, bold=True)
...
def create_ping_table(bot_ping, db_ping, cache_ping):
    try:
        ...
        # Load fonts
        try:
            text_font = load_ui_font(24, text="Bot Ping", bold=True)
        except IOError:
...
def create_relation_percentage_banner(
    ...
    text = f"{percentage}%"
    font = load_ui_font(50, text=text, bold=True)
...
def _image_fallback_placeholder() -> Image.Image:
    try:
        if _IMAGE_FALLBACK_PATH.exists():
            return Image.open(_IMAGE_FALLBACK_PATH).convert("RGBA")
    except Exception:
        pass
    return Image.new("RGBA", (200, 200), (255, 255, 255, 0))


def _log_image_warning_once(url: str, message: str):
    key = str(url or "").strip() or "<empty>"
    if key in _WARNED_IMAGE_LOAD_URLS:
        return
    _WARNED_IMAGE_LOAD_URLS.add(key)
    logger.warning(message)


def _is_discord_attachment_url(raw_url: str) -> bool:
    return "cdn.discordapp.com/attachments/" in str(raw_url or "")

def load_image_from_url(url: str) -> Image.Image:
    """Load an image from a URL or local path and convert it to RGBA."""
    def _strip_discord_query(raw_url: str) -> str:
        raw = str(raw_url or "").strip()
        if "cdn.discordapp.com" not in raw or "?" not in raw:
            return raw
        try:
            parts = urlsplit(raw)
    ...
    try:
        ...
        fallback_url = _strip_discord_query(str(url))
        if fallback_url and fallback_url != str(url):
            try:
                fallback_response = requests.get(fallback_url, stream=True, timeout=12)
                fallback_response.raise_for_status()
                return Image.open(fallback_response.raw).convert("RGBA")
            except Exception as fallback_error:
                if _is_discord_attachment_url(url):
                    _log_image_warning_once(
                        str(url),
                        f"Discord attachment image unavailable (primary={e}, fallback={fallback_error}); using local fallback. url={url}",
                    )
                else:
                    _log_image_warning_once(str(url), f"Error loading image: primary={e} fallback={fallback_error} url={url}")
                return _image_fallback_placeholder()
        if _is_discord_attachment_url(url):
            _log_image_warning_once(str(url), f"Discord attachment image unavailable; using local fallback. url={url}")
        else:
            _log_image_warning_once(str(url), f"Error loading image: {e} url={url}")
        return _image_fallback_placeholder()
...
def create_simple_music_banner(
    ...
    try:
        ...
        title_font = load_ui_font(36, text=music_title, bold=True)
        artist_font = load_ui_font(24, text=music_author, bold=False)
        time_font = load_ui_font(20, bold=False)
